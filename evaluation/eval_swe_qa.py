...
async def index_repositories(assistant: Assistant, repos: List[str], index_config: dict):
    """
    ...
    for idx, repo_url_with_commit in enumerate(repos):
        ...
        try:
            ...
            if response.meta.status == "done":
                ...
                print(f"  Failed to index {repo_url_with_commit}: {response.job_status.description_error}")
                
        except Exception as e:
            print(f"  Exception occurred during indexing of {repo_url_with_commit}: {e}")
...
async def eval_single_repo(
    ...
    for idx, row in questions.iterrows():
        ...
        try:
            ...
            print(f"    ✗ Error: {error_msg[:200]}...")  # Показываем первые 200 символов
...
async def eval(repo_meta_path: Path, question_path: Path, output_path: Path = None):
    ...
    Args:
        ...
        output_path: Path to output JSONL file (default: results.jsonl in question_path directory)
    """
    assistant = Assistant(
    ...
    for repo_url_with_commit in repos:
        ...
        # parts[0]="https:", parts[1]="", parts[2]="github.com", parts[3]="django", parts[4]="django"
    ...
    if question_path.is_dir():
        for question_file in question_path.iterdir():
            ...
            if not repo_url:
                print(f"  ⚠ Warning: Could not find repo URL for {repo_name}, skipping...")
                continue
...
if __name__ == "__main__":
    ...
    parser.add_argument("--repo_meta", type=Path, required=True, help="Path to file with repository metadata")
    parser.add_argument("--question_path", type=Path, required=True, help="Path to directory with question files")
    parser.add_argument("--output", type=Path, required=False, help="Path to output JSONL file")
