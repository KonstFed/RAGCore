...
class LLMGenerationParams(BaseModel):
    ...
    presence_penalty: float = Field(
        0.0,
        ge=-2.0, le=2.0,
        description="Штраф за повторное появление токенов."
    )
...
class LLMConfig(BaseModel):
    ...
    system_prompt: Optional[str] = Field(
        "Ты - ассистент для ответов на вопросы по репозиторию с кодом. Тебе дан диалог с пользователем и релевантные контексты по некоторому репозиторию.",
...
class ContentBlockingSettings(BaseModel):
    """Настройки полной замены ответа/запроса при обнаружении нежелательных паттернов."""
    enabled: bool = False
...
class RegexSubstitutionRule(BaseModel):
    ...
    replacement: str = Field(..., description="Строка, на которую нужно заменить найденный паттерн.")
...
class QueryPostprocessorConfig(BaseModel):
    ...
    blacklist: Optional[ContentBlockingSettings] = Field(
        None, description="Если модель сгенерировала запрещенный контент, заменить ответ."
    )
...
class RewriterTemplates(BaseModel):
    ...
    context_template: str = Field(
        "Filepath: {metadata.filepath}, start line number: {metadata.start_line_no}, end line number: {metadata.end_line_no}\n\n{content}",
...
class FilterCondition(BaseModel):
    ...
    name: Literal["filepath", "file_name", "language", "chunk_size", "node_count", "start_line_no"]
    value: Union[str, int, float, bool, List[Any]] = Field(..., description="Значение для сравнения.")
    operator: Literal["eq", "neq", "gt", "gte", "lt", "lte", "in", "wildcard", "contains"]
