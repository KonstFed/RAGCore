...
class LLMClient:
    ...
    def __init__(self, cfg: DictConfig) -> None:
        ...
        if hasattr(cfg, "llm"):
            try:
                self.default_llm_config = LLMConfig.model_validate(OmegaConf.to_object(cfg.llm))
    ...
    def agenerate(
        ...
        if not cfg:
            raise RuntimeError("LLM config is not provided and default config is missing.")

        base_url = cfg.base_url or "https://openrouter.ai/api/v1"
        api_key = os.getenv("OPENROUTER_API_KEY") or ""  # конфиг тоже допускает api_key, но env приоритетнее
        ...
        # OpenRouter рекомендует добавлять Referer и HTTP-User-Agent, но сделаем опционально
        ...
        if response.status_code != 200:
            raise RuntimeError(f"LLM request failed: status={response.status_code}, body={response.text}")
